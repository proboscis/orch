...
ORCH_BIN="${ORCH_BIN:-${HOME}/.local/bin/orch}"
...
MESSAGE="${MESSAGE:-matrix-remote-message}"
...
"$ORCH_BIN" --remote "$ZEUS_HOST:7777" --project "$PROJECT_ID" send "$ISSUE_ID#$RUN_ID" "$MESSAGE" >/dev/null
