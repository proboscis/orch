...
set +e
RUN_OUT="$(ssh "$ZEUS_HOST" "cd '$ZEUS_PROJECT_ROOT' && $ZEUS_ORCH_BIN --project '$PROJECT_ID' run '$ISSUE_ID' --run-id '$RUN_ID' --on '$TARGET_WORKER_ID' --agent custom --agent-cmd 'printf target-e2e-ready; hostname; sleep 20' --multiplexer tmux --json" 2>&1)"
RUN_RC=$?
set -e
printf '%s\n' "$RUN_OUT"
if [ "$RUN_RC" -ne 0 ]; then
