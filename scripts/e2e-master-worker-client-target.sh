...
echo "RUN_ID=$RUN_ID"
echo "REMOTE_ORCH_BIN=$REMOTE_ORCH_BIN"
echo "TARGET_ORCH_BIN=$TARGET_ORCH_BIN"
echo "TARGET_ENV_PREFIX=$TARGET_ENV_PREFIX"

eval "$TARGET_SSH_CMD \"$TARGET_ENV_PREFIX $TARGET_ORCH_BIN --remote= daemon repo register '$TARGET_REPO'\""
eval "$TARGET_SSH_CMD \"$TARGET_ENV_PREFIX ORCH_REMOTE=$REMOTE_HOST:7777 $TARGET_ORCH_BIN worker run --worker-id '$TARGET_WORKER_ID'\" >'$TARGET_WORKER_LOG' 2>&1" &
TARGET_WORKER_PID=$!

cleanup() {
  set +e
  if [ -n "${PROJECT_ID:-}" ]; then
    ssh "$REMOTE_HOST" "$REMOTE_ORCH_BIN --project '$PROJECT_ID' stop '$ISSUE_ID#$RUN_ID' --force" >/dev/null 2>&1 || true
  fi
  ssh "$REMOTE_HOST" "rm -f '$REMOTE_ISSUES_PATH/issues/$ISSUE_ID.md'" >/dev/null 2>&1 || true
...
REGISTER_OUT="$(ssh "$REMOTE_HOST" "cd '$REMOTE_PROJECT_ROOT' && $REMOTE_ORCH_BIN daemon repo register '$REMOTE_PROJECT_ROOT'")"
printf '%s\n' "$REGISTER_OUT"
...
set +e
RUN_OUT="$(ssh "$REMOTE_HOST" "cd '$REMOTE_PROJECT_ROOT' && $REMOTE_ORCH_BIN --project '$PROJECT_ID' run '$ISSUE_ID' --run-id '$RUN_ID' --on '$TARGET_NAME' --agent custom --agent-cmd 'printf target-e2e-ready; hostname; sleep 20' --multiplexer tmux --json" 2>&1)"
RUN_RC=$?
set -e
printf '%s\n' "$RUN_OUT"
if [ "$RUN_RC" -ne 0 ]; then
...
WORKER_JSON="$(ssh "$REMOTE_HOST" "$REMOTE_ORCH_BIN worker status --json")"
printf '%s\n' "$WORKER_JSON"
printf '%s' "$WORKER_JSON" | jq -e --arg worker "$TARGET_WORKER_ID" '.workers | any(.id == $worker)' >/dev/null

PS_OUT="$(ssh "$REMOTE_HOST" "$REMOTE_ORCH_BIN --project '$PROJECT_ID' ps --issue '$ISSUE_ID' --json")"
printf '%s\n' "$PS_OUT"
...
SHOW_OUT="$(ssh "$REMOTE_HOST" "$REMOTE_ORCH_BIN --project '$PROJECT_ID' show '$ISSUE_ID#$RUN_ID' --json")"
printf '%s\n' "$SHOW_OUT"
