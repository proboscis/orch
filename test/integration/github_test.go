...
func startDaemon(t *testing.T, binary, vault, repo string) func() {
	t.Helper()
	cmd := exec.Command(binary, "--vault", vault, "daemon")
	cmd.Dir = vault
...
func TestGitHubBackend_ListIssues_Empty(t *testing.T) {
	...
	cmd := exec.Command(binary, "--vault", vault, "issue", "list", "--json")
	out, err := cmd.CombinedOutput()
	if err != nil {
...
func TestGitHubBackend_CreateAndListIssue(t *testing.T) {
	...
	createCmd := exec.Command(binary, "--vault", vault, "issue", "create", "--title", testTitle, "--body", testBody, "--json")
	...
	listCmd := exec.Command(binary, "--vault", vault, "issue", "list", "--json")
...
func TestGitHubBackend_SyncIssues(t *testing.T) {
	...
	syncCmd := exec.Command(binary, "--vault", vault, "issue", "sync", "--json")
...
`
	...
	listCmd := exec.Command(binary, "--vault", vault, "issue", "list")
