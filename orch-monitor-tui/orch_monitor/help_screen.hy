...
(defclass OnboardingScreen [(get ModalScreen bool)]
  ...
  (defn compose [self]
    (with [(Vertical :id "onboarding-dialog")]
      ...
      (when (and self.config_state.has_orch_dir (not self.config_state.has_config_file))
        (yield (Static "[yellow]Missing:[/] .orch/config.yaml")))
  ...
  (defn _check_configuration [self]
    ...
    (when (and state.has_orch_dir state.has_config_file)
