...
func modelRunToProto(run *model.Run) *orchpb.Run {
	...
	protoRun := &orchpb.Run{
		IssueId:           sanitizeUTF8(run.IssueID.String()),
		RunId:             sanitizeUTF8(run.RunID.String()),
...
func protoRunToModel(run *orchpb.Run) *model.Run {
	...
	return &model.Run{
		IssueID:           model.NewIssueID(run.IssueId),
		RunID:             model.NewRunID(run.RunId),
...
func modelIssueToProto(issue *model.Issue) *orchpb.Issue {
	...
	return &orchpb.Issue{
		Id:             sanitizeUTF8(issue.ID.String()),
