...
func TestGetRunByShortIDAmbiguous(t *testing.T) {
	...
	ids := []string{run1.ShortID().String(), run2.ShortID().String(), run3.ShortID().String()}
	...
	if ambiguousPrefix != "" {
		...
		_, err := s.GetRunByShortID(model.NewShortID(ambiguousPrefix))
		if err == nil {
...
func TestGetRunByShortIDAmbiguousForced(t *testing.T) {
	...
	for i := 0; i < 20; i++ {
		...
		run, err := s.CreateRun("test", model.NewRunID(runID), nil)
		if err != nil {
...
func TestGitHubIssueIDAliasing(t *testing.T) {
	...
	run, err := s.GetLatestRun(model.NewIssueID(canonicalIssueID))
	if err != nil {
	...
	runs, err := s.ListRuns(&store.ListRunsFilter{IssueID: model.NewIssueID(canonicalIssueID)})
	if err != nil {
	...
	ref := &model.RunRef{IssueID: model.NewIssueID(canonicalIssueID), RunID: model.NewRunID(runID)}
	run, err = s.GetRun(ref)
	if err != nil {
...
`)
	...
	for _, issue := range issues {
		issueMap[issue.ID.String()] = issue
	}
...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			issue, err := s.ResolveIssue(model.NewIssueID(issueID))
			if err != nil {
				t.Fatalf("ResolveIssue() error = %v", err)
			}
