...
func ParseRunRef(s string) (RunRef, error) {
	...
	if shortIDRegex.MatchString(s) {
		return RunRef{ShortID: model.NewShortID(s)}, nil
	}

	lastHash := strings.LastIndex(s, "#")
	if lastHash == -1 {
		return RunRef{IssueID: model.NewIssueID(s)}, nil
	}

	candidate := s[lastHash+1:]
	if looksLikeRunID(candidate) {
		return RunRef{
			IssueID: model.NewIssueID(s[:lastHash]),
			RunID:   model.NewRunID(candidate),
		}, nil
	}
	return RunRef{IssueID: model.NewIssueID(s)}, nil
}
