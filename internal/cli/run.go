...
Debug output can be enabled with --verbose, --log-level debug, or ORCH_DEBUG=1.`,
	...
	cmd.Flags().StringVar(&opts.Model, "model", "", "Model (opencode: provider/model e.g. anthropic/claude-opus-4-5; claude: model name e.g. claude-fable-5)")
	cmd.Flags().StringVar(&opts.ModelVariant, "model-variant", "", "Model variant (opencode: 'max' for max thinking; claude: effort low|medium|high|xhigh|max)")
